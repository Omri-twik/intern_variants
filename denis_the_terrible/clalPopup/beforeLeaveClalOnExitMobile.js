...
function resetDistanceScrolledUp() {
...
function setButtonFunctionalities() {
  ...
  clalActivationButtons.forEach((btn) => {
    if (!btn.classList.contains("twik_popup_listener")) {
      ...
      btn.addEventListener("click", resetPopExitFunctionality);
    ...
    if (!btn.classList.contains("twik_popup_listener")) {
      ...
      btn.addEventListener("click", () => {
        ...
        if (
          ...
          resetPopExitFunctionality();
        } else {
          deactivatePopUpExitFunctionality();
        }
      });
      btn.classList.add("twik_popup_listener");
    }
...
function resetPopExitFunctionality() {
  // console.log("resetPopExitFunctionality");
  resetDistanceScrolledUp();
  deactivatePopUpExitFunctionality();
  activatePopExitFunctionality();
}

function activatePopExitFunctionality() {
  // console.log("activatePopExitFunctionality");
  resetDistanceScrolledUp();
  popExitTimeout = setTimeout(startTrackingScrollUp, 3000);
}

function deactivatePopUpExitFunctionality() {
  // console.log("deactivatePopUpExitFunctionality");
  resetDistanceScrolledUp();
  try {
    clearTimeout(popExitTimeout);
  } catch {}
  try {
    stopTrackingScrollUp();
  } catch {}
}
...
function scrollTracking() {
  ...
  if (oldValue - newValue > 0) {
    ...
    if (distanceScrolledUp >= scrollLimit) {
      // alert("popup");
