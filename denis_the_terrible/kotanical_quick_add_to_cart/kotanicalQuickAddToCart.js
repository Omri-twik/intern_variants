...
function mainJS() {
          ...
          function (module, __webpack_exports__, __webpack_require__) {
            ...
            function unique(path) {
              switch (rootDocument.querySelectorAll(selector(path)).length) {
                case 0:
                  throw new Error(
                    `Can't select any node with this selector: ${selector(
                      path
                    )}`
                  );
                case 1:
            ...
            function classNames(input) {
              const names = Array.from(input.classList).filter(
                config.className
              );
                ...
                if (tags[tag].length > maxCount) {
                  ...
                  tags[tag].forEach(function (el) {
                    ...
                    if (commonClasses.length) {
                      commonClasses = commonClasses.filter(function (
                        className
                      ) {
  ...
  function create_addtc_buttons(selector) {
    ...
    for (let prod_index in prods) {
      let prod = prods[prod_index];

      // search for url that takes us to items's product page and append ".json"
      let data_url = create_data_finding_url(selector, prod_index); // done
      ...
      prod.insertAdjacentHTML(
        ...
        `
          <button style="text-decoration: underline;">
              <a class="add_to_cart_btn" tw-ref="${data_url}" style="text-decoration-color: #c07282; font-size: 1.3em; color:black">ADD TO CART</a>
              <a class="adding_to_cart_btn" style="display:none; text-decoration-color: #c07282; font-size: 1.3em; color:black">Adding...</a>
          </button>
          `
      );

      // on any button click
      let addToCartBtn = prod.querySelector(".add_to_cart_btn");
      let addingToCartBtn = prod.querySelector(".adding_to_cart_btn");
  ...
  function find_and_add_product(json_str) {
    ...
    if (document.cookie.indexOf("cart=") != -1) {
      let prod_chain = `/cart/${variant_id}:1`;
    ...
    // only for the first addtc - after this the cart is initiated
      ...
      document.body.insertAdjacentHTML(
        ...
        <div style="display:none;">
          <form class="tw-form" method=" post" action="/cart/add">
      ...
      // document.querySelector("#tw-form").submit();
    }
  }
  ...
  if (added == "true") {
    ...
    console.log(
      "[-] Twik added no previous item added, cart staying hidden..."
    );
  }
  ...
  let has_addtc = check_for_addtc(page_products_selector);

  if (has_addtc == true) {
    console.log("[-] Add to cart button already detected by Twik");
  } else {
    console.log("[+] Twik creating add to cart buttons...");
