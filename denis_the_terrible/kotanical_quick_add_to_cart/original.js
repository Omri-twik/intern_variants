...
function create_addtc_buttons(selector) {
  ...
  for (let prod in prods) {
    let temp_item = prods[prod];

    // search for url that takes us to items's product page and append ".json"
    let data_url = create_data_finding_url(selector, prod); // done

    // create addtc button
    let add_btn = document.createElement("button");
    add_btn.innerHTML = `<a>ADD TO CART</a>`;
    add_btn.style.textDecoration = "underline";
    add_btn.firstElementChild.style.textDecorationColor = "#c07282";
    add_btn.firstElementChild.style.fontSize = "1.3em";
    add_btn.firstElementChild.style.color = "black";
    temp_item.style.textAlign = "center";
    add_btn.firstElementChild.setAttribute("tw-ref", data_url);
