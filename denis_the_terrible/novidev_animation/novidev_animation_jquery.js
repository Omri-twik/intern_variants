...
function mainJS() {
  ...
  let $tables = $(
    "body > div.main-container > section:nth-child(5) > div > div > div > div > div > div:nth-child(3) > div > table"
  );
  ...
  function toggleTable($tbody) {
    if ($tbody.hasClass("tableOpen")) {
      console.log("off");
      $tbody.removeClass("tableOpen");
      $tbody.animate({ height: "0px" });
    } else {
      console.log("on");
      $tbody.addClass("tableOpen");
      $tbody.animate({ height: $tbody.css("scrollHeight") });
    }
  }
}
